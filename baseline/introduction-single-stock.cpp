...
Type objective_function<Type>::operator() () {
    ...
    array<Type> detail_fish__num(10,1,5,as_integer(total_steps + (double)(1))); detail_fish__num.setZero();
    array<Type> detail_fish__wgt(10,1,5,as_integer(total_steps + (double)(1))); detail_fish__wgt.setConstant((double)(1));
    ...
    array<Type> detail_fish__predby_f_surv(10,1,5,as_integer(total_steps + (double)(1)));
    array<Type> detail_fish__renewalnum(10,1,5,as_integer(total_steps + (double)(1))); detail_fish__renewalnum.setZero();
    array<Type> detail_fish_f_surv__cons(10,1,5,as_integer(total_steps + (double)(1)));
    Type nll = (double)(0);
    ...
    while (true) {
            ...
            for (auto age = fish__minage; age <= fish__maxage; age++) {
                ...
                if ( area == f_surv__area ) {
...

                    {
                        ...
                        fish_f_surv__suit.col(fish__age_idx).col(fish__area_idx) = ((double)(1) / ((double)(1) + exp(-fish__f_surv__alpha*(fish__midlen - fish__f_surv__l50))))*fish__num.col(fish__age_idx).col(fish__area_idx)*fish__wgt.col(fish__age_idx).col(fish__area_idx);
        ...
        if ( report_detail == 1 ) {
            detail_fish__predby_f_surv.col(cur_time + 1 - 1) = fish__predby_f_surv;
        }
        if ( report_detail == 1 ) {
            detail_fish__renewalnum.col(cur_time + 1 - 1) = fish__renewalnum;
        }
        if ( report_detail == 1 ) {
            detail_fish_f_surv__cons.col(cur_time + 1 - 1) = fish_f_surv__cons;
        }
        if ( cur_step_final ) {
            // g3a_age for fish;
