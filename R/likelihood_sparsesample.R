...
g3l_sparsesample <- function (
    ...
    # nll_(csparse|asparse)_(nll_name)__(mean|stddev|obs_n|sum|sqsum|model_n)
    # nll_(csparse|asparse)_(nll_name)__nll
    nllstock <- g3s_sparsedata(c("nll", type = if (length(predstocks) > 0 ) "csparse" else "asparse", name = nll_name), obs_df[,-ncol(obs_df), drop = FALSE], area_group = area_group)
