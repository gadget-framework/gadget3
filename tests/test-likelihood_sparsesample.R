...
for (i in seq_len(nrow(obs_linreg_df))) {
    ...
    count <- r$hist_stst__num[
        ...
        time = sprintf("%04d-%02d", obs_linreg_df[i, "year"], obs_linreg_df[i, "step"]) ]
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_bt__model_sum[i, '1994-02']),
        sum(m * count) ), paste0("r$hist_nll_asparse_bt__model_sum[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_bt__model_sqsum[i, '1994-02']),
        sum(m**2 * count) ), paste0("r$hist_nll_asparse_bt__model_sqsum[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_bt__model_n[i, '1994-02']),
        sum(count) ), paste0("r$hist_nll_asparse_bt__model_n[", i, ", '1994-02']"))
}
ok(ut_cmp_equal(
    r$hist_nll_asparse_bt__model_sum,
    ifelse(r$hist_nll_asparse_bt__model_n > 0, r$hist_nll_asparse_bt__model_sum[,"1994-02"], 0),
    end = NULL), "hist_nll_asparse_bt__model_sum: History is the same shape as __model_n")
ok(gadget3:::ut_cmp_df(as.data.frame(r$hist_nll_asparse_bt__model_n > 0), '
...
'), "hist_nll_asparse_bt__model_n: Added value once per step, apart from 1993")
ok(ut_cmp_equal(
    r$hist_nll_asparse_bt__obs_mean,
    array(
        obs_linreg_df$mean,
        dim = c(vec = length(obs_linreg_df$mean), time = 10),
        dimnames = list(vec = NULL, time = paste0(rep(1990:1994, each=2), c("-01", "-02"))) ),
    end = NULL), "hist_nll_asparse_bt__obs_mean: Repeats the input observations")
nll_mean <- r$hist_nll_asparse_bt__model_sum[, '1994-02']/r$hist_nll_asparse_bt__model_n[, '1994-02']
ok(ut_cmp_equal(
    r$nll_asparse_bt__nll,
    gadget3:::regression_linear(nll_mean, obs_linreg_df$mean, NaN, 1) ), "r$nll_asparse_bt__nll: Matches derived version")

# cs_model
ok(ut_cmp_equal(colSums(r$hist_nll_asparse_cs_model__model_n) / colSums(r$hist_stst__num, dims = 2), c(
    "1990-01" = 1,
    "1990-02" = 2,
    "1991-01" = 2,
    "1991-02" = 2,
    ...
    "1994-01" = 6,
    "1994-02" = 6,
    NULL )), "hist_nll_asparse_cs_model__model_n: Final data point has 6 step's worth of individuals")
...
for (i in seq_len(nrow(obs_ss_df))) {
    ...
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_cs_model__model_sum[i, '1994-02']),
        sum(m * count) ), paste0("r$hist_nll_asparse_cs_model__model_sum[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_cs_model__model_sqsum[i, '1994-02']),
        sum(m**2 * count) ), paste0("r$hist_nll_asparse_cs_model__model_sqsum[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_asparse_cs_model__model_n[i, '1994-02']),
        sum(count) ), paste0("r$hist_nll_asparse_cs__model_n[", i, ", '1994-02']"))
}
nll_var <- r$hist_nll_asparse_cs_model__model_sqsum[, '1994-02']/r$hist_nll_asparse_cs_model__model_n[, '1994-02'] -
    (r$hist_nll_asparse_cs_model__model_sum[, '1994-02']/r$hist_nll_asparse_cs_model__model_n[, '1994-02'])**2
nll_mean <- r$hist_nll_asparse_cs_model__model_sum[, '1994-02']/r$hist_nll_asparse_cs_model__model_n[, '1994-02']
ok(ut_cmp_equal(
    (r$nll_asparse_cs_model__nll),
    array((nll_mean - obs_ss_df$mean)^2 * 1/nll_var, dim = c(row = 3L), dimnames = list(row = NULL)) ), "r$nll_asparse_cs_model__nll: Matches derived version")

# Overall nll
ok(ut_cmp_equal(nll,
    params$asparse_linreg_bt_weight * r$nll_asparse_bt__nll[["nll"]] +
    params$asparse_sumsquares_cs_model_weight * sum(r$nll_asparse_cs_model__nll) +
    params$csparse_sumsquares_flc_weight * sum(r$nll_csparse_flc__nll) +
...
for (i in seq_len(nrow(obs_ss_df))) {
    ...
    count <- (r$hist_stst_fl_surv__cons / r$hist_stst__wgt)[
        ...
        time = sprintf("%04d-%02d", obs_flc_df[i, "year"], 1:2) ]
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_csparse_flc__model_n[i, '1994-02']),
        sum(count) ), paste0("r$hist_nll_csparse_flc__model_n[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_csparse_flc__model_sum[i, '1994-02']),
        sum(m * count) ), paste0("r$hist_nll_csparse_flc__model_sum[", i, ", '1994-02']"))
    ok(ut_cmp_equal(
        as.vector(r$hist_nll_csparse_flc__model_sqsum[i, '1994-02']),
        sum(m**2 * count) ), paste0("r$hist_nll_csparse_flc__model_sqsum[", i, ", '1994-02']"))
}

nll_var <- obs_flc_df$stddev^2  # Using weighting = 'obs_stddev'
nll_mean <- r$hist_nll_csparse_flc__model_sum[, '1994-02']/r$hist_nll_csparse_flc__model_n[, '1994-02']
ok(ut_cmp_equal(
    r$nll_csparse_flc__nll,
    array((nll_mean - obs_flc_df$mean)^2 * (1/nll_var) * obs_flc_df$number, dim = c(row = length(r$nll_csparse_flc__nll)), dimnames = list(row = NULL)) ), "r$nll_csparse_flc__nll: Matches derived version")
