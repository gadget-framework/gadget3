...
param_model <- function (...) {
    ...
    predstock <- g3_fleet(c(country = 'is', 'comm')) %>% g3s_age(1, 5) %>% g3s_livesonareas(areas[c('a', 'c')])
...
ok(ut_cmp_identical(param_tmpl(
    ...
    NULL)$switch, c(
        ...
        "had_m_imm.paramut.1993.c", "had_m_imm.paramut.1994.c" )), "by_area: Generate parameters with name")
...
print( attr(model_fn(params), "had_m_imm_is_comm__num")[,age='age1',,pred_age='age1',] )
