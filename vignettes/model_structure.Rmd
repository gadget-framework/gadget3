...
ling_model <- g3_to_r(list(
    g3a_age(ling_imm),
    g3a_age(ling_mat),
    g3a_mature(ling_imm, ling_mat, maturity_f),
    time))
```

All steps within the actions you provide will be ordered by name, and any
repeated names will be removed. Actions will give each step names based on the
standard order of operations for Gadget, although these can be altered by the
common ``run_at`` parameter if needed.
...
Variables that the stock uses within the model will be named based on the name
given to ``g3_stock()``, in the case above ``ling_imm__num`` and
``ling_imm__wgt`` will be defined.

When you use an action such as ``g3a_growmature()``, you provide formulae to fill in
the gaps. These will iterate over any dimensions the stock has, such as age and
area, and will be available to you when writing these formulae.
...
TODO: Most of this is internal to gadget3, I'm not sure when/if it should be
exposed and allow others to have at it.
...
substitutions, which are handled by ``stock_step()``. This takes a formula,
...
```{r, eval=FALSE}
    ...
    out[[step_id(run_at, 1, stock)]] <- stock_step(f_substitute(~{
        ...
        stock_iterate(stock, stock_intersect(matured, if (run_f) {
            ...
            matured__num[matured__iter] <- stock__num[stock__iter] * maturity_f
            stock__num[stock__iter] <- stock__num[stock__iter] - matured__num[matured__iter]
        }))
...
dimensions, and replace ``[stock__iter]`` with a subset that will provide a
