...
``g3_collate()`` just orders actions by name, the actions themselves set the
name based on the standard order of actions. This can be altered with the
``run_at`` parameter if required.
...
When you use an action such as ``g3a_grow()``, you provide formulae to fill in
...
Next ``stock_rename()`` is used to to transform ``matured__wgt <- stock__wgt``
