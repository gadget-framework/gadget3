...
gadget3 itself is not an ecosystem model.
Rather, it's main aim is to produce code that can be converted into other forms,
...
As opposed to other modelling frameworks there is no input data formats,
the model configuration is written as an R script.
...
NB: This and all subsequent sections of the model script are marked in blue.
...
The other key concept in a gadget3 model is a *stock*.
These do not directly correspond to a species, more complicated models may have many stocks with independent processes representing a species.
...
The main aim of the stock functions is to define the structure for arrays that will hold stock state.
We can produce an example of an array using ``g3_stock_instance()``:
...
Next we add population dynamics actions for our stock into our ``actions`` array from earlier:
...
Each of these ``g3a_*`` actions will have a 1:1 parallel with gadget2 components,
...
* ``g3a_renewal_normalparam()``: Ongoing recruitment at every timestep (``run_step = NULL``), see the [Recruitment](#recruitment) section below
...
The order of these actions as we have defined them is not preserved, they will be re-ordered to match the standard action order, see `?g3_action_order`.

Likelihood actions are actions that will sum their output into the model's overall likelihood score,
More will be defined later, when we deal with fleets.

#### Recruitment

There are 2 recruitment processes you can employ in a gadget3 (or gadget2) model;
``g3a_renewal_normalparam()`` which introduces new individuals into a stock regardless of the current state of the stock,
and ``g3a_spawn()``, which adds individuals based on the current size & state of the stock.

TODO: Further description of using g3a_spawn at this point?
...
For more detail on what can be done here, see [Supplying data to gadget3](supplying-data-to-gadget3.html).
