...
``g3_to_r()`` will convert a list of actions into R code.
...
```{r}
simple_fn <- g3_to_r(list(g3a_time(1990, 1991), g3a_initialconditions_normalcv(fish)))
...
abund <- attr(simple_fn(params), "fish__num")

par(mfrow=c(3, 2), mar = c(2,2,1,0))
for (a in dimnames(abund)$age) barplot(abund[,area = 'IXa', age = a], main = a)
```
